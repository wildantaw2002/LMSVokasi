...
<div class="container mx-auto max-w-2xl px-4 py-8">
    <h1 class="text-3xl font-bold mb-8">Tambah Kelas Baru</h1>
        ...
        <form action="{{ route('admin.kelas.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="kode_kelas" id="kode_kelas" class="w-full px-3 py-2 border border-gray-300 rounded" 
                    value="{{ old('kode_kelas') }}" placeholder="contoh: CS101" required>
                @error('kode_kelas')
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nama_kelas" id="nama_kelas" class="w-full px-3 py-2 border border-gray-300 rounded" 
                    value="{{ old('nama_kelas') }}" placeholder="contoh: Pemrograman Web" required>
                @error('nama_kelas')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <select name="instruktur_id" id="instruktur_id" class="w-full px-3 py-2 border border-gray-300 rounded" required>
                    ...
                    @foreach($gurus as $guru)
                        <option value="{{ $guru->id }}" {{ old('instruktur_id') === (string)$guru->id ? 'selected' : '' }}>
                            {{ $guru->name }}
                        </option>
                    @endforeach
                </select>
